    case PE_IndicatorMenuCheckMark: {
        ...
        // doesn't matter.
        Swatchy fgColor = Phantom::Color::S_windowText;
        bool isSelected = option->state & (State_Selected | State_On);
        bool isEnabled = option->state & State_Enabled;
        if (isSelected) {
            fgColor = Phantom::Color::S_highlightedText;
        } else if (!isEnabled) {
            fgColor = Phantom::Color::Phantom::Color::S_windowText_disabled;
        }
        qreal rx, ry, rw, rh;
    ...
    case PE_PanelTipLabel: {
        ...
        const qreal radius = Ph::DefaultFrame_Radius;
        painter->save();
    ...
    case PE_FrameTabBarBase: {
        ...
        fillRectEdges(painter, option->rect, edge, 1, swatch.color(Phantom::Color::S_frame_outline));
        ...
        fillRectEdges(painter, Ph::expandRect(option->rect, edge, -1), edge, 1, swatch.color(S_tabFrame_specular));
        break;
    }
    case PE_PanelScrollAreaCorner: {
        ...
        if (isLeftToRight) {
            ...
            edges |= Qt::RightEdge;
            bgRect.setWidth(bgRect.size().width() - 1);
        }
    ...
    case PE_IndicatorHeaderArrow: {
        ...
        if (!header)
            return;
        QRect r = header->rect;
        QPoint offset = QPoint(Ph::HeaderSortIndicator_HOffset, Ph::HeaderSortIndicator_VOffset);
        qreal lightness = Ph::DeriveColors::hack_isLightPalette(widget->palette()) ? 0.03 : 0.0;
    ...
    case PE_IndicatorToolBarSeparator: {
        ...
        if (option->state & State_Horizontal) {
            if (r.height() >= 10)
                r.adjust(0, 3, 0, -3);
            r.setWidth(r.size().width() / 2 + 1);
            fillRectEdges(painter, r, Qt::RightEdge, 1, swatch.color(Phantom::Color::S_window_divider));
        } else {
    ...
    case PE_PanelButtonTool: {
        ...
        paintBorderedRoundRect(painter, r.adjusted(1, 1, -1, -1), rounding, swatch, specular, 0);
        break;
    }
    ...
    case PE_FrameLineEdit: {
        ...
        PSave save(painter);
        paintBorderedRoundRect(painter, r, rounding, swatch, pen, 0);
        save.restore();
    ...
    case PE_PanelLineEdit: {
        ...
        painter->setRenderHint(QPainter::Antialiasing);
        painter->setPen(swatch.pen(Ph::SwatchColors::0));
    ...
    case PE_IndicatorCheckBox: {
        ...
        Swatchy fgColor = isFlat ? Phantom::Color::S_windowText : S_text;
        if (isPressed && !isFlat) {
            fgColor = Phantom::Color::S_highlightedText;
        }
    ...
    case PE_IndicatorRadioButton: {
        ...
        if (Ph::IndicatorShadows && !isSunken && isEnabled) {
            ...
            painter->setPen(Qt::NoPen);
            painter->setBrush(swatch.color(S_base_shadow));
        ...
        if (state & State_On) {
            Swatchy fgColor = isSunken ? Phantom::Color::S_highlightedText : Phantom::Color::S_windowText;
    ...
    case PE_IndicatorToolBarHandle: {
        ...
        for (int row = 0; row < rows; ++row) {
            for (int col = 0; col < columns; ++col) {
                ...
                painter->fillRect(x_, y_, dotLen, dotLen, swatch.color(Phantom::Color::S_window_divider));
            }
        }
        break;
    }
    ...
    case PE_FrameFocusRect: {
                        ...
                        if (selectionModel) {
                            ...
                            if (selection.count() == 1) {
                                ...
                                if (isTableView) {
                                    ...
                                    if ((range.size().width() == 1 && range.height() == 1)
        ...
        paintBorderedRoundRect(
            painter, option->rect, Ph::FrameFocusRect_Rounding, swatch, S_highlight_outline, 0);
        break;
    }
    ...
    case PE_FrameTabWidget: {
        ...
        if (!twf)
            break;
        fillRectOutline(painter, option->rect, 1, swatch.color(Phantom::Color::S_frame_outline));
    ...
    case Phantom_PE_IndicatorTabNew: {
        Swatchy fg = Phantom::Color::S_windowText;
        Swatchy bg = 0;
        if ((option->state & State_Enabled) && (option->state & State_MouseOver)) {
            fg = Phantom::Color::S_highlightedText;
        ...
        painter->setRenderHint(QPainter::Antialiasing);
        QRect r = option->rect;
        // int adj = (int)((qreal)qMin(r.size().width(), r.height()) * (1.0 / 2.5));
    ...
    case PE_PanelMenu: {
        ...
        // fillRectOutline(painter, option->rect, 1, swatch.color(Phantom::Color::S_window_divider));
        ...
        painter->drawRoundedRect(frameRect, Ph::DefaultFrame_Radius, Ph::DefaultFrame_Radius);

        // blur
        if (widget && widget->window()) {
//            QPainterPath path;
//            path.addRoundedRect(option->rect, Ph::DefaultFrame_Radius, Ph::DefaultFrame_Radius);
//            const_cast<QWidget *>(widget)->setMask(path.toFillPolygon().toPolygon());
//            m_blurHelper->update(const_cast<QWidget *>(widget));

            m_blurHelper->enableBlurBehind(const_cast<QWidget *>(widget), true, Ph::DefaultFrame_Radius);
        }
    ...
    case Phantom_PE_ScrollBarSliderVertical: {
        ...
        if (isLeftToRight) {
            ...
            mainRect.setWidth(mainRect.size().width() - 1);
        }
...
static qreal CheckMark_WidthOfHeightScale = 1.0;
    ...
    const Ph::Phantom::Color& swatch = *ph_swatchPtr.data();
    ...
    case CE_ComboBoxLabel: {
        ...
        if (!cb->currentIcon.isNull()) {
            ...
            iconRect.setWidth(cb->iconSize.size().width() + 4);
            ...
            if (cb->direction == Qt::RightToLeft)
                editRect.translate(-4 - cb->iconSize.size().width(), 0);
            else
                editRect.translate(cb->iconSize.size().width() + 4, 0);
        }
    ...
    case CE_Splitter: {
        ...
        if (option->state & State_Horizontal) {
            ...
            size = QSize(r.size().width() / 2, length);
            radius = r.size().width() * 0.3;
        } else {
            if (r.size().width() < length)
                length = r.size().width();
        ...
        // fillRectOutline(painter, filledRect.adjusted(-1, 0, 1, 0), 1, swatch.color(Phantom::Color::S_window_divider));
        break;
    }
    ...
    case CE_DockWidgetTitle: {
        ...
        if (verticalTitleBar) {
            QRect r = dwOpt->rect;
            QRect rtrans = {r.x(), r.y(), r.height(), r.size().width()};
            titleRect = QRect(rtrans.left() + r.bottom() - titleRect.bottom(),
                              rtrans.top() + titleRect.left() - r.left(),
                              titleRect.height(),
                              titleRect.size().width());
            painter->translate(rtrans.left(), rtrans.top() + rtrans.size().width());
            painter->rotate(-90);
            painter->translate(-rtrans.left(), -rtrans.top());
        }
        if (!dwOpt->title.isEmpty()) {
            QString titleText = painter->fontMetrics().elidedText(dwOpt->title, Qt::ElideRight, titleRect.size().width());
            proxy()->drawItemText(painter,
                                  titleRect,
    ...
    case CE_HeaderSection: {
        ...
        painter->fillRect(bgRect, swatch.color(S_window));
        fillRectEdges(painter, rect, edges, 1, swatch.color(Phantom::Color::S_frame_outline));
        break;
    }
    case CE_HeaderLabel: {
        auto header = qstyleoption_cast<const QStyleOptionHeader*>(option);
        ...
        if (!header->icon.isNull()) {
            int iconExtent = qMin(qMin(rect.height(), rect.size().width()), option->fontMetrics.height());
            ...
            int pixw = static_cast<int>(pixmap.size().width() / pixmap.devicePixelRatio());
            ...
            painter->drawPixmap(inter.x(),
                                ...
                                inter.x() - aligned.x(),
                                inter.y() - aligned.y(),
                                static_cast<int>(aligned.size().width() * pixmap.devicePixelRatio()),
    ...
    case CE_ProgressBarContents: {
        ...
        if (isIndeterminate || bar->progress > bar->minimum) {
            ...
            paintBorderedRoundRect(
                painter, filled.adjusted(1, 1, -1, -1), rounding, swatch, S_progressBar_specular, 0);
            if (isIndeterminate) {
    ...
    case CE_ProgressBarLabel: {
        ...
        if (needsFilled) {
            ...
            painter->setPen(swatch.pen(Phantom::Color::S_highlightedText));
            painter->setBrush(Qt::NoBrush);
            painter->drawText(textRect, bar->text, Qt::AlignHCenter | Qt::AlignVCenter);
            if (needsMasking) {
                painter->restore();
    ...
    case CE_MenuBarItem: {
        ...
        if (Ph::MenuBarDrawBorder && !isSelected) {
            fillRectEdges(painter, r, Qt::BottomEdge, 1, swatch.color(Phantom::Color::S_window_divider));
        }
        break;
    }
    ...
    case CE_MenuItem: {
        ...
        if (isSelected) {
            ...
            qreal item_radius = Ph::DefaultFrame_Radius / 2;
            painter->save();
            painter->setPen(Qt::NoPen);
        ...
        if (isCheckable) {
            ...
            Swatchy signColor = !isEnabled ? Phantom::Color::S_windowText : isSelected ? Phantom::Color::S_windowText /*Phantom::Color::S_highlightedText*/ : Phantom::Color::S_windowText;
            if (menuItem->checkType & QStyleOptionMenuItem::Exclusive) {
                ...
                if (isChecked) {
                    ...
                    qreal dim = qMin(checkRect.size().width(), checkRect.height()) * 0.75;
        ...
        if (hasIcon) {
            ...
            int iconExtent = qMin(iconRect.size().width(), iconRect.height());
            ...
            const int pixw = static_cast<int>(pixmap.size().width() / pixmap.devicePixelRatio());
        ...
        if (!s.isEmpty()) {
            ...
            if (!styleHint(SH_UnderlineShortcut, menuItem, widget))
                ...
                painter->save();
#endif
            painter->setPen(swatch.pen(isSelected ? S_text /*Phantom::Color::S_highlightedText*/ : S_text));
        ...
        if (hasSubMenu) {
            ...
            Swatchy arrowColor = isSelected ? Phantom::Color::S_highlightedText : Phantom::Color::S_indicator_current;
            drawArrow(painter, arrowRect, arrow, swatch.color(arrowColor));
        }
        painter->restore();
        break;
    }
    ...
    case CE_PushButtonLabel: {
        ...
        if (!button->icon.isNull()) {
            ...
            int pixmapWidth = static_cast<int>(pixmap.size().width() / pixmap.devicePixelRatio());
            int pixmapHeight = static_cast<int>(pixmap.height() / pixmap.devicePixelRatio());
            int labelWidth = pixmapWidth;
            int labelHeight = pixmapHeight;
            // 4 is hardcoded in QPushButton::sizeHint()
            int iconSpacing = 4;
            int textWidth = button->fontMetrics.boundingRect(option->rect, tf, button->text).size().width();
            if (!button->text.isEmpty())
                labelWidth += (textWidth + iconSpacing);
            iconRect = QRect(textRect.x() + (textRect.size().width() - labelWidth) / 2,
            ...
            if (button->direction == Qt::RightToLeft)
                ...
                textRect.setLeft(iconRect.left() + iconRect.size().width() + iconSpacing);
    ...
    case CE_MenuBarEmptyArea: {
        ...
        if (Ph::MenuBarDrawBorder) {
            fillRectEdges(painter, rect, Qt::BottomEdge, 1, swatch.color(Phantom::Color::S_window_divider));
        }
    ...
    case CE_TabBarTabShape: {
        ...
        if (hasFrame) {
            ...
            if (isSelected) {
                ...
                specular = Ph::TabBar_InactiveTabsHaveSpecular ? S_inactiveTabYesFrame_specular : 0;
            }
        } else {
            ...
            if (isSelected) {
                ...
                specular = Ph::TabBar_InactiveTabsHaveSpecular ? S_inactiveTabNoFrame_specular : 0;
            }
        }
        auto frameColor = isSelected ? Phantom::Color::S_frame_outline : S_window_outline;
        paintBorderedRoundRect(painter, drawRect, rounding, swatch, frameColor, thisFillColor);
        paintBorderedRoundRect(painter, drawRect.adjusted(1, 1, -1, -1), rounding, swatch, specular, 0);
        painter->restore();
        if (isSelected) {
    ...
    case CE_ShapedFrame: {
        ...
        if (frameopt) {
            if (frameopt->frameShape == QFrame::HLine) {
                ...
                r.setHeight(1);
                painter->fillRect(r, swatch.color(Phantom::Color::S_frame_outline));
                break;
            } else if (frameopt->frameShape == QFrame::VLine) {
                QRect r = option->rect;
                r.setX(r.x() + r.size().width() / 2);
                r.setWidth(2);
                painter->fillRect(r, swatch.color(S_tabFrame_specular));
                r.setWidth(1);
                painter->fillRect(r, swatch.color(Phantom::Color::S_frame_outline));
...
static qreal CheckMark_WidthOfHeightScale = 1.0;
    ...
    case CC_SpinBox: {
        ...
        if (spinBox->frame) {
            ...
            painter->save(); // 0
            // Fill background
            paintBorderedRoundRect(painter, rect, rounding, swatch, 0, S_base);
            // Draw button fill
            painter->setClipRect(upDownRect);
            // Side with the border
            Qt::Edge edge = isLeftToRight ? Qt::LeftEdge : Qt::RightEdge;
            paintBorderedRoundRect(
                painter, Ph::expandRect(upDownRect, Ph::oppositeEdge(edge), -1), rounding, swatch, 0, S_button);
            painter->restore(); // 0
            if (Ph::OverhangShadows && !hasFocus && isEnabled) {
            ...
            paintBorderedRoundRect(painter, rect, rounding, swatch, outlineColor, 0);
            save.restore();
        }
        ...
        if (spinBox->buttonSymbols == QAbstractSpinBox::PlusMinus) {
            ...
            Swatchy arrowColorUp =
                spinBox->stepEnabled & QAbstractSpinBox::StepUpEnabled ? Phantom::Color::S_indicator_current : S_indicator_disabled;
            Swatchy arrowColorDown =
                spinBox->stepEnabled & QAbstractSpinBox::StepDownEnabled ? Phantom::Color::S_indicator_current : S_indicator_disabled;
    ...
    case CC_TitleBar: {
        ...
        QString title = painter->fontMetrics().elidedText(titleBar->text, Qt::ElideRight, textRect.size().width() - 14);
    ...
    case CC_ScrollBar: {
        ...
        scrollBarSlider.setWidth(scrollBarSlider.size().width() - padding);
        ...
        if (scrollBar->subControls & SC_ScrollBarSlider) {
            qreal radius =
                (scrollBar->orientation == Qt::Horizontal ? scrollBarSlider.height() : scrollBarSlider.size().width()) / 2.0;
            ...
            painter->setRenderHint(QPainter::Antialiasing);
            painter->setPen(Qt::NoPen);
            painter->setBrush(mouseOver ? swatch.color(S_scrollbarSlider_hover) : swatch.color(S_scrollbarSlider));
            if (mousePress)
                painter->setBrush(swatch.color(S_scrollbarSlider_pressed));
    ...
    case CC_ComboBox: {
        ...
        if (comboBox->subControls & SC_ComboBoxArrow) {
            int margin =
                static_cast<int>(qMin(downArrowRect.size().width(), downArrowRect.height()) * Ph::ComboBox_ArrowMarginRatio);
    ...
    case CC_Slider: {
        ...
        if (option->subControls & SC_SliderTickmarks) {
            ...
            while (v <= slider->maximum + 1) {
                ...
                int pos = sliderPositionFromValue(slider->minimum,
                                                  slider->maximum,
                                                  v_,
                                                  (horizontal ? slider->rect.size().width() : slider->rect.height()) - len,
                                                  slider->upsideDown)
                          + len / 2;
        ...
        if ((option->subControls & SC_SliderHandle)) {
            ...
            r.adjust(1, 1, -1, -1);
            paintBorderedRoundRect(painter, r, Ph::SliderHandle_Rounding, swatch, handleSpecular, 0);
        }
        break;
    }
    case CC_ToolButton: {
...
int BaseStyle::pixelMetric(PixelMetric metric, const QStyleOption* option, const QWidget* widget) const
{
    ...
    case PM_DefaultFrameWidth:
        ...
        val = qMax(1, Ph::DefaultFrameWidth - 2);
        break;
    ...
    case PM_SmallIconSize:
        if (Ph::ItemView_UseFontHeightForDecorationSize && widget
...
static qreal CheckMark_WidthOfHeightScale = 1.0;
    ...
    case CT_CheckBox: {
        ...
        return QSize(size.size().width() + w + margins, qMax(size.height(), h));
    }
    case CT_MenuBarItem: {
        ...
        // return QSize(size.size().width() + w * 2, size.height() + h * 2 + line);
    }
    case CT_MenuItem: {
        auto menuItem = qstyleoption_cast<const QStyleOptionMenuItem*>(option);
        if (!menuItem)
    ...
    case CT_Slider: {
        ...
        if (!widget || qobject_cast<const QSlider*>(widget)->orientation() == Qt::Horizontal) {
            ...
            sz.setWidth(sz.size().width() + PM_SliderTickmarkOffset);
        }
        return sz;
    }
    case CT_GroupBox: {
        ...
        if (opt->subControls & (SC_GroupBoxCheckBox | SC_GroupBoxLabel)) {
            ...
            yadd += static_cast<int>(fontHeight * Ph::GroupBox_LabelBottomMarginFontRatio);
        }
        ...
        return QSize(size.size().width() + xadd, size.height() + yadd);
    }
    case CT_ItemViewItem: {
        ...
        sz += QSize(0, Ph::DefaultFrameWidth);
...
#if 1
        if (Ph::ItemView_UseFontHeightForDecorationSize) {
    ...
    case CT_HeaderSection: {
        ...
        sz.setWidth((nullIcon ? 0 : margin) + iconSize + (hdr->text.isNull() ? 0 : margin) + txt.size().width() + margin);
        if (hdr->sortIndicator != QStyleOptionHeader::None) {
            if (hdr->orientation == Qt::Horizontal)
                sz.rwidth() += sz.height() + margin;
            else
                sz.rheight() += sz.size().width() + margin;
        }
        return sz;
    }
    default:
    ...
    case CT_PushButton: {
        ...
        if (!pbopt || pbopt->text.isEmpty())
            break;
        int hpad = static_cast<int>(pbopt->fontMetrics.height() * Ph::PushButton_HorizontalPaddingFontHeightRatio);
        newSize.rwidth() += hpad * 2;
        if (widget && qobject_cast<const QDialogButtonBox*>(widget->parent())) {
            int dialogButtonMinWidth = Ph::dpiScaled(80);
            newSize.rwidth() = qMax(newSize.size().width(), dialogButtonMinWidth);
        }
        break;
    }
    case CT_ToolButton:
#if defined(Q_OS_MACOS)
        newSize += QSize(Ph::dpiScaled(6 + Ph::DefaultFrameWidth), Ph::dpiScaled(6 + Ph::DefaultFrameWidth));
#elif defined(Q_OS_WIN)
        newSize += QSize(Ph::dpiScaled(4 + Ph::DefaultFrameWidth), Ph::dpiScaled(4 + Ph::DefaultFrameWidth));
#else
        newSize += QSize(Ph::dpiScaled(3 + Ph::DefaultFrameWidth), Ph::dpiScaled(3 + Ph::DefaultFrameWidth));
#endif
        break;
    case CT_ComboBox: {
        newSize += QSize(0, Ph::dpiScaled(4 + Ph::DefaultFrameWidth));
        auto cb = qstyleoption_cast<const QStyleOptionComboBox*>(option);
...
static qreal CheckMark_WidthOfHeightScale = 1.0;
    ...
    case CC_Slider: {
        ...
        case SC_SliderHandle: {
            if (slider->orientation == Qt::Horizontal) {
                ...
                int centerX = slider->rect.center().x() - rect.size().width() / 2;
                if (slider->tickPosition & QSlider::TicksAbove)
                    centerX += tickSize;
                if (slider->tickPosition & QSlider::TicksBelow)
                    centerX -= tickSize;
    ...
    case CC_SpinBox: {
        ...
        int x, lx, rx;
        x = spinbox->rect.size().width() - y - buttonWidth + 2;
        lx = fw;
        rx = x - fw;
        switch (subControl) {
        case SC_SpinBoxUp:
        ...
        case SC_SpinBoxEditField:
            if (spinbox->buttonSymbols == QAbstractSpinBox::NoButtons) {
                rect = QRect(lx, fw, spinbox->rect.size().width() - 2 * fw, spinbox->rect.height() - 2 * fw);
            } else {
    ...
    case CC_GroupBox: {
        ...
        case SC_GroupBoxLabel: {
            ...
            case Qt::AlignHCenter:
                x += (option->rect.size().width() - contentWidth) / 2;
                break;
            case Qt::AlignRight:
                x += option->rect.size().width() - contentWidth;
                break;
            default:
                break;
            }
    ...
    case CC_ComboBox: {
        ...
        int dim = qMin(r.size().width(), r.height());
        if (dim < 1)
            return QRect();
        switch (subControl) {
        ...
        case SC_ComboBoxArrow: {
            QRect r0 = r;
            r0.setX((r0.x() + r0.size().width()) - dim + 1);
...
QPixmap BaseStyle::generatedIconPixmap(QIcon::Mode iconMode, const QPixmap& pixmap, const QStyleOption* opt) const
{
    ...
    if (iconMode == QIcon::Selected) {
        ...
        painter.fillRect(0, 0, img.size().width(), img.height(), color);
                         ...
                         QStyleHintReturn* returnData) const
{
    ...
    case SH_ItemView_ShowDecorationSelected:
        ...
        if (!Ph::ShowItemViewDecorationSelected && qobject_cast<const QListView*>(widget))
            return 1;
    ...
    case SE_TreeViewDisclosureItem: {
        if (BranchesOnEdge) {
            ...
            if (opt->direction != Qt::RightToLeft) {
                rect.moveLeft(0);
                if (rect.size().width() < rect.height())
